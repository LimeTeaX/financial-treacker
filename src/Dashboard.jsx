  ...
  return (
    <article className="rounded-3xl bg-white p-6 border border-slate-100 shadow-sm flex flex-col gap-4">
      <div className="flex items-start justify-between">
        <span className={`inline-flex h-11 w-11 items-center justify-center rounded-2xl ${color} ${iconColor}`}>
          {icon}
        </span>
      ...
      <div>
        ...
        <p className="mt-2 flex items-center gap-1.5 text-xs">
          <span className={`font-semibold ${positive ? "text-emerald-500" : "text-rose-500"}`}>{change}</span>
...
export default function Dashboard() {
  ...
  const stats = useMemo(() => {
    ...
    return [
      ...
      {
        label: "Investment",
        value: `Rp ${Math.max(0, balance * 0.3).toLocaleString('id-ID')}`,
...
const handleSubmit = async (e) => {
  ...
  const success = await addTransaction({ 
    ...
    amount,
    type: form.type  // ← PASTIKAN INI ADA!
  })
        ...
        <div className="rounded-3xl bg-white p-6 border border-slate-100 shadow-sm">
          <div className="flex items-center justify-between mb-6">
            ...
            <select value={txSort} onChange={(e) => setTxSort(e.target.value)}
              ...
              <option>Newest</option><option>Oldest</option><option>Highest</option><option>Lowest</option>
            </select>
          </div>
                ...
                <tr key={tx.id} className="hover:bg-slate-50/60 transition-colors">
                  <td className="py-3.5 pr-4">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <span className="font-medium text-slate-800 text-sm">{tx.merchant}</span>
  ...
  {tx.type === 'income' ? '+' : '-'}Rp {Math.abs(tx.amount || 0).toLocaleString("id-ID")}
</td>
                    ...
                    <div className="relative">
                      ...
                      {openMenuId === tx.id && (
                        <div className="absolute right-0 top-8 bg-white rounded-2xl border border-slate-100 shadow-lg z-30 py-2 w-32">
                          <button onClick={() => {
                            setEditingTx(tx)
                            setEditForm({ merchant: tx.merchant, category: tx.category || "Food", amount: String(Math.abs(tx.amount || 0)) })
                            setOpenMenuId(null)
                          }} className="w-full text-left px-4 py-2 text-sm text-slate-600 hover:bg-slate-50">Edit</button>
                          <button onClick={async () => {
                            const success = await deleteTransaction(tx.id)
                            if (success) showToast("Transaksi berhasil dihapus!", "success")
                            setOpenMenuId(null)
                          }} className="w-full text-left px-4 py-2 text-sm text-rose-500 hover:bg-rose-50">Delete</button>
                        </div>
                      )}
                    </div>
                  </td>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div>
                ...
                <input type="text" value={form.merchant} onChange={(e) => setForm({ ...form, merchant: e.target.value })}
                  placeholder="Warung Just Is Resto" required
              ...
              <div>
                ...
                <select value={form.category} onChange={(e) => setForm({ ...form, category: e.target.value })}
                  ...
                  <option>Internet</option><option>Subscription</option><option>Shopping</option>
                  <option>Salary</option><option>Education</option><option>Entertainment</option>
                </select>
              </div>
              <div>
              ...
              <div className="flex gap-3 pt-2">
                ...
                <button type="submit"
                  className="flex-1 rounded-xl bg-[#8B5CF6] py-2.5 text-sm font-medium text-white hover:bg-violet-700">Add Transaction</button>
              </div>
            </form>
          </div>
        </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm">
          <div className="bg-white rounded-3xl p-6 w-full max-w-md border border-slate-100 shadow-xl">
            <div className="flex items-center justify-between mb-4">
              ...
              <button onClick={() => setEditingTx(null)} className="text-slate-400 hover:text-slate-600"><X size={20} /></button>
            </div>
            <form onSubmit={async (e) => {
              ...
              const success = await updateTransaction(editingTx.id, { merchant: editForm.merchant, category: editForm.category, amount: finalAmount })
              if (success) { showToast("Transaksi berhasil diupdate!", "success"); setEditingTx(null) }
              else { showToast("Gagal mengupdate transaksi!", "error") }
            }} className="space-y-4">
              <div>
                <label className="text-xs font-semibold text-slate-500 uppercase tracking-wider">Merchant</label>
              ...
              <div className="flex gap-3 pt-2">
                <button type="button" onClick={() => setEditingTx(null)}
                  className="flex-1 rounded-xl border border-slate-200 py-2.5 text-sm font-medium text-slate-600 hover:bg-slate-50">Cancel</button>
                <button type="submit"
                  className="flex-1 rounded-xl bg-[#8B5CF6] py-2.5 text-sm font-medium text-white hover:bg-violet-700">Save Changes</button>
              </div>
            </form>
          </div>
        </div>
