...
export default function Dashboard() {
  ...
  const handleSubmit = async (e) => {
    ...
    await addTransaction({ merchant: form.merchant, category: form.category, amount });
              ...
              {sortedTxns.map((tx) => (
                <tr key={tx.id} className="hover:bg-slate-50/60 transition-colors">
                  ...
                  <td className="py-3.5 text-right font-bold text-slate-800">
                    Rp {Math.abs(tx.amount || 0).toLocaleString("id-ID")}
                  </td>
  ...
  const saved = await updateTransaction(editingTx.id, {
    merchant: editForm.merchant,
    category: editForm.category,
    amount: finalAmount,
  })
  
  if (saved) setEditingTx(null)
}} className="space-y-4">
              <div>
                <label className="text-xs font-semibold text-slate-500 uppercase tracking-wider">Merchant</label>
